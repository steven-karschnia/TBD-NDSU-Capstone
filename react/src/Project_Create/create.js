/*TO USE CREATE PAGE: Make sure to import necessary packages 
then change the ReactDom.render() from <Main /> to <CreatePage />
I don't know another way to make the page functional */
...
$(function () {
  $.fn.setFlexGrid = function (options) {
    var defaults = {
      ...
      cols: 12, // amount of columns across a grid
      rows: 12, // amount of rows in a grid
    ...
    $.fn.height = function (type, value) {
      ...
      // stuff I will be extending that doesn't affect/change the way .height() works
    ...
    $.fn.addWidget = function (params) {
      ...
      var widget = noParams
        ? $(
            '<div class="fg-widget"><i class="fa fa-chevron-right fg-resize-widget" aria-hidden="true"></i><i class="fa fa-times fg-remove-widget" title="remove this widget."></i><div class="fg-widget-inner fg-widget-handle"><div class="zone-txt text-center"></div></div></div>'
          )
        : params.widget === undefined || params.widget === null
        ? $(
            '<div class="fg-widget"><i class="fa fa-chevron-right fg-resize-widget" aria-hidden="true"></i><i class="fa fa-times remove-widget" title="remove this widget"></i><div class="fg-widget-inner fg-widget-handle"><div class="zone-txt text-center"></div></div></div>'
          )
        : params.widget;
      ...
      goHere = zo.obj;
      this.sibs = goHere.siblings(".fg-col");
    ...
    $.fn.removeWidget = function (widget) {
      ...
      if (!widget.hasClass("cloner")) {
        // for the widgets that only have one instance
        var de = widget.detach();
        de.appendTo(".widget-holder");
        de.css({
    ...
    $.fn.addRowHere = function (goHere, rowsNeeded) {
      ...
      this.lastCol = zoneInner.find(".fg-col").last(); // reset since we added rows
      rowCount = parseInt(this.lastCol.attr("data-fg-row")) + 1; // reset since we added rows
      zoneInner.height(rowCount * re.rH); // reset since we added rows
      re.zone.height(zoneInner.height() + 50); // reset since we added rows
      enableSortable();
    };
      ...
      $(".fg-col").sortable({
        ...
        receive: function (event, ui) {
          ...
          if (!ui.item.data("sortableItem").bindings.hasClass("fg-col")) {
            ...
            var ogHeight = ui.item.data("sortableItem").helperProportions
              .height;
            ui.item.data({
  ...
  $(document).on("click", ".fg-add-widget", function () {
    ...
    var widget = $(
      '<div class="fg-widget"><i class="fa fa-chevron-right fg-resize-widget"></i><i class="fa fa-times fg-remove-widget" title="remove this widget"></i><i class="fas fa-arrows-alt move-widget fg-widget-handle"></i><div class="fg-widget-inner" style="background: #406fff !important;"></div></div>'
    );
  ...
  $(".widget-holder").sortable({
    ...
    appendTo: "body",
    handle: ".fg-widget-handle",
    placeholder: "fg-widget-placeholder ui-corner-all",
    scroll: false,
    start: function (event, ui) {
      if (ui.item.hasClass("cloner")) {
        clone = ui.item.clone();
        clone.appendTo(".widget-holder");
    ...
    // stop: function (event, ui) {
    //   if (ui.item.parent().hasClass("widget-holder")) {
    //     ui.item.css({
    //       width: "80px",
    //       height: "80px",
    //       "min-width": "80px",
    ...
    //     !ui.item.parent(".widget-holder").length
    //   ) {
    //     clone.appendTo(".widget-holder");
    //     clone.css("display", "inline-block");
    //   }
    ...
    return (
      <div>
        <div>
          ...
          <div class="col-sm-12">
            <div class="row">
              <div class="grid-settings" /*style={{maxwidth: "300px;"}}*/>
                {/*</div><button class="btn button display-inline new_widget">new widget at </button>*/}
              </div>
            </div>
            <div
              class="row"
                  ...
                  >
                    <div class="flexgrid-helper">
                      <div class="form-check display-inline view-gridlines-div">
                        <label class="view-gridlines-label">
                          <strong>Display gridlines?</strong>
                        </label>
                        <div class="toggle">
                          <input
                            type="checkbox"
                            ...
                            class=" togglegridlines"
                            checked
                          />
                          <label for="toggle1"></label>
                        </div>
                      </div>
                      <button class="btn btn-sm clear-flexgrid">
                        Clear Grid
                      </button>
                      <button class="btn btn-sm save-flexgrid">
        ...
        <label class="widget-holder-label">Toolbox:</label>
        <div class="widget-holder">
          <div class="fg-widget custom-widget cloner">
            <div class="arrow">
              <i
                class="fa fa-chevron-right fg-resize-widget"
                aria-hidden="true"
              ></i>
              <i
                class="fa fa-times fg-remove-widget"
                title="remove this widget"
              ></i>
              <i class="fa fa-arrows-alt move-widget fg-widget-handle"></i>
              <div class="fg-widget-inner">
                <i class="fa fa-arrow-right fa-4x inner-icon"></i>
              </div>
            </div>
          </div>
          <div class="fg-widget custom-widget">
            <div class="pre-project">
              ...
              <i class="fa fa-arrows-alt move-widget fg-widget-handle"></i>
              <div class="fg-widget-inner">
                <i class="fas fa-ad inner-icon">Pre-Project</i>
              </div>
            </div>
          </div>
          <div class="fg-widget custom-widget">
            <div class="init">
              ...
              <i class="fa fa-arrows-alt move-widget fg-widget-handle"></i>
              <div class="fg-widget-inner">
                <i class="fas fa-ad inner-icon">Init</i>
              </div>
            </div>
          </div>
          <div class="fg-widget custom-widget cloner">
            <div class="stages">
              ...
              <i class="fa fa-arrows-alt move-widget fg-widget-handle"></i>
              <div class="fg-widget-inner">
                <i class="fas fa-ad inner-icon">Stage</i>
              </div>
            </div>
          </div>
          <div class="fg-widget custom-widget cloner">
            <div class="sprints">
              ...
              <i class="fa fa-arrows-alt move-widget fg-widget-handle"></i>
              <div class="fg-widget-inner">
                <i class="fas fa-ad inner-icon">Sprint</i>
              </div>
            </div>
          </div>
          <div class="fg-widget custom-widget cloner">
            <div class="forms">
              ...
              <i class="fa fa-arrows-alt move-widget fg-widget-handle"></i>
              <div class="fg-widget-inner">
                <i class="fas fa-ad inner-icon" style={{ textAlign: "center" }}>
                  Form
                  <textarea type="text" class="txtbox" />
                </i>
              </div>
            </div>
          </div>
